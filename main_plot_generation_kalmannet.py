...
from src.estimate_kalman_net_joint import EstimateKalmanNetJoint
from src.train_kalman_net import train_kalman_net
import numpy as np
import torch
import os
...
# Training 
fixed_parameters = np.array([20.0, 120.0, -84.0, -60.0])
delta_t = 1e-1 # ms

no_timesteps_training = 100 # Not necessarily the same as the one for inference
dataset_size = 500
device = 'cuda' if torch.cuda.is_available() else 'cpu'
models_folder = 'models'
model_name = f'KNet_T_{no_timesteps_training}_N_{dataset_size}.pt'

train_kalman_net(models_folder,
                     model_name,
                     fixed_parameters,
                     delta_t,
                     no_timesteps_training,
                     dataset_size,
                     device)
...
#folder = f'results/kalman_net_T_{no_timesteps_training}_N_{dataset_size}_est_T_{no_timesteps}_with_legend'
folder = f'results/kalman_net_T_{no_timesteps_training}_N_{dataset_size}_est_T_{no_timesteps}_no_legend'
os.makedirs(folder, exist_ok=True)

# UKF estimator initialization
state_dim = 2 + 8 # 2 states + 8 parameters

model_path = os.path.join(models_folder, model_name)
estimator = EstimateKalmanNetJoint(model_path, device=device)
